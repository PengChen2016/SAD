...
f_Rmetal=[6.32, 6.56, 6.68, 6.31, 6.82, 6.97, 6.9, 7.15, 7.61];
...
for i_p=1:3
    for i_Pin=1:3
        ...
        fem.PTE(i_p,1,i_Pin)=f_Rmetal(i_data);
        fem.Pplasma(i_p,1,i_Pin)=f_Ls(i_data);
    end
end
...
% 导入csv
radialHztypical= csvread('radial_Hz_typical.csv',1,0);
fem.radialHztypical_r=radialHztypical(:,1)/1e3;
radialHztypical(radialHztypical(:,2)==0,2)=nan;
fem.radialHztypical_Hz=radialHztypical(:,2);
