...
verifyEqual(testCase,nonuniform_dist.get_ne_z([200;z]),ref_dist.ne_z(1:15,2),'Abs',0.01)
verifyEqual(testCase,nonuniform_dist.get_Te_z(z),ref_dist.Te_z(1:14,2),'Abs',0.03)
r=flip([45, 40, 35, 30, 25, 20, 15, 10, 5, 0]');
verifyEqual(testCase,nonuniform_dist.get_ne_r([r;45.5]),ref_dist.ne_r(1:11,2),'Abs',0.02)
verifyEqual(testCase,nonuniform_dist.get_Te_r(r),ref_dist.Te_r(1:10,2),'Abs',0.01)
% test get average value
z=[0,200;-200,0;-200,200;-130,130];
actual_mean=nonuniform_dist.get_ne_z(z);
excepted_mean=0.421*ones(3,1);
verifyEqual(testCase,actual_mean(1:3),excepted_mean,'Rel',0.04)
actual_mean=nonuniform_dist.get_Te_z(z);
excepted_mean=10*trapz(ref_dist.Te_z(:,2))/260;
verifyEqual(testCase,actual_mean(4),excepted_mean,'Rel',0.01)
r=[0,45.5;0,45];
actual_mean=nonuniform_dist.get_ne_r(r);
...
actual_mean=nonuniform_dist.get_Te_r(r);
...
ratio_origin2goal
verifyEqual(testCase,nonuniform_dist.get_ne_r([0,45.5])/nonuniform_dist.get_ne_r(10),ratio_origin2goal.ne_r,'Rel',0.01)
end
