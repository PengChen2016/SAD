...
fprintf('[INFO] Results from electric model.\n');
    if 1==source.size
        %% single point
        disp('等离子体等效阻抗')
        fprintf('%s = %.2e Ω, ','PER',source.PER);
        fprintf('%s = %.2e , ','Lplasma',source.Lplasma);
        fprintf('%s = %.2e \n','Xplasma',source.Xplasma);
        disp('系统等效阻抗Zsys')
        fprintf('%s = %.2e , ','Rsys',source.Rsys);
        fprintf('%s = %.2e , ','Lsys',source.Lsys);
        fprintf('%s = %.2e \n','Xsys',source.Xsys);
